...
- 用户输入两个名字；
- 每个名字经 **MD5** 确定该斗士的技能、称号、元素与技能个性化；属性为固定基础值；
...
1. 斗士的一切派生数据 = `f(归一化后的名字, 当前配置快照)` 的**纯函数**。
   名字与配置不变，则技能 / 称号 / 元素 / 属性（固定值 + 称号加成）**永远不变**，
   与运行次数、进程、机器无关。
...
5. 派生时主 PRNG 的消耗顺序固定为：
   **元素 -> 技能数量 -> 技能抽取 -> 称号结构 -> 称号字段（按结构顺序）**。
   属性为固定基础值（无随机），稀有度系统已删除（v0.5.0）。改变顺序属于
   breaking change（见 3.4）。
   **数值类随机一律使用高斯分布**（`DetRng.next_gaussian`，Box-Muller，
   每次消耗两个均匀数；离散选择仍用加权均匀抽取）；适用范围：技能数量、
   熟练度、个性化扰动倍率、共鸣倍率、伤害浮动。
   **数值精度**：后台运算不取整、全程保留浮点（离散计数如持续刻数除外）；
   一切展示中**百分数保留 2 位小数、其余数值保留整数**
   （后端 `format_pct` / `format_num` 与前端格式化遵循同一规则，v0.8.0 起）。
   **白板 100 基准**（v0.9.0）：hp/atk/def/spd 基础值均为 100（加成可超 100），
   战斗公式以 `battle.json` 的 `atk_factor` / `defense_factor` / `gauge_threshold`
   折算回与旧体系等价的实际当量（数值实际不变、量纲归一）。
   称号字段附带小额属性加成（`game/titles.json` 各字段 `bonus`，白板量纲），
   查表叠加到固定属性，不消耗随机数。
...
2. **文案**全部位于 `config/locales/<lang>/*.json`（每语言九个文件）：UI 文案、
   ...
   称号字段（titles）、元素、buff 文案（buffs）、技能词缀（modifiers）、
   战斗日志模板（battle_log）。
...
│       ├── zh/               # ui/attributes/elements/skills/titles/stats/buffs/modifiers/battle_log
│       └── en/
├── web/                      # 前端（无构建、零依赖）
│   ├── index.html
│   ├── css/style.css
...
- **tick 战斗模型**：战斗以 tick 推进；每个 tick 双方行动槽（gauge）累加自身有效
  速度，达到阈值（`battle.json` 的 `gauge_threshold`，默认 1000；白板速度 100 ≈
  每 10 刻一动）即可行动一次并扣回阈值。速度决定行动频率。
  同一 tick 多人可行动时按（gauge 余量降序、内部序）执行；内部序 = 速度降序、
  规范化名字升序，与输入顺序无关；
- **白板 100 量纲**（v0.9.0）：伤害公式 `max(最小伤害, ATK × atk_factor × 浮动 ×
  暴击倍率 × 技能倍率 − 有效防御 × (1−穿透) × defense_factor)`；改属性 base 必须
  同比例改折算常数；元素不参与计算（仅身份标识）；
...
- **变量共鸣**（v0.9.0 双槽位）：每个效果类型在 `variable_link.targets` 声明
  ...
  触发时按归一化系数修正技能自身参数，公式括号紧跟对应数值
  （`基数 + 变量式*合并系数`），变量式用属性 emoji（❤️⚔️🛡️💨🎯🌀，own 省略
  范围词、enemy 前缀「对方」、差值/并值【己方-对方】/【己方+对方】）；
  名称后缀标记（锋/坚/疾/命/锐/影）依变数顺序追加；对战中前端按快照实时计算
  最终值（`link_calc` 列表 + `\u0001` 标记逐个替换），支持简易显示模式；
- **称号加成**：称号各字段自带小额属性加成（可为负，白板量纲），叠加到固定
  基础属性，卡牌展示聚合结果（「称号加成：生命 +3 · 速度 +10」）；
