...
- **变量共鸣**：可共鸣类型的技能（`variable_link.linkable_types`）有概率与一项属性
  （atk/def/spd/hp/crit/dodge，己方或敌方、比例或差值模式）建立共鸣，触发时按
  归一化系数修正**技能自身的某个参数**（`variable_link.targets` 指定字段，
  如 stun -> chance）；共鸣变量与倍率均由个性化种子决定，并以名称后缀标记体现
  （如「重击·命」= 生命共鸣，标记文案在 locale 的 `stats.json` `link_*` 键）；
  描述内联最简线性公式（`基数 +（变量式）× 合并系数`），变量以「当前」值参与；
  对战中前端按快照实时计算最终值（后端提供 `link_calc` 公式数据），
  并支持简易显示模式（隐藏公式括号）；
- **称号加成**：称号各字段自带小额属性加成（可为负），叠加到固定基础属性，
