...
输入两个名字，MD5 决定它们的一切——技能、称号、元素与共鸣，以及一场完全确定的战斗（属性为固定基础值，差异来自称号与技能）。
**相同的名字，永恒的命运。**
...
2. **派生**：`md5(归一化名字)` 作为 splitmix64 PRNG 的种子，按固定顺序抽出元素、2–3 个技能（高斯分布的技能数量）与组合式称号；属性为固定基础值 + 称号加成，稀有度系统已移除；数值类随机（技能数/扰动/共鸣倍率/伤害浮动）全部服从高斯分布；
3. **技能个性化**：每个技能的触发概率与数值再以 `md5(名字:技能id)` 为种子做确定性扰动（区间见 `skills.json` 的 `md5_variance`）--同名技能在不同名字手中强弱不同；
4. **名称词缀**：技能有概率获得前缀/后缀（如「疾风·重击·破军」），词缀附带触发率、效果值、毒伤或持续的小幅修正（可为负的权衡设计）；
5. **变量共鸣**：攻击类技能 95% 概率与一项属性建立共鸣，**来源（己方/敌方）与模式（比例/差值）均由 MD5 决定**：比例 = 属性当前值 × 倍率，差值 = (己方属性 − 敌方参照) × 倍率（攻↔防、速↔速等有意义配对）；倍率随战斗动态变化（如生命共鸣随掉血衰减）。技能名后缀体现共鸣变量（「淬毒之刃·坚」= 防御）。全部公式见 `docs/GAME_SPEC.md`；
6. **称号生成与加成**：按权重抽取结构（单核心 / 前缀+核心 / 核心+后缀 / 双核心 / 全结构）与各字段，拼接成如「孤高术士·候补」的称号；每个字段自带小额属性加成（可为负，如「候补」攻击-1），卡牌展示聚合结果；
7. **对战**：以 **tick** 推进--每 tick 双方行动槽累加速度值，满阈值（100）即可行动；速度决定出手频率。伤害计算含暴击/闪避/技能触发/共鸣附伤/毒/眩晕/反伤/追击，行动刻耗尽按剩余生命比例判定；
8. **逐刻回放与悬停详情**：回放按游戏刻逐刻演出（85ms/刻 ÷ 倍速），行动槽每刻推进、战报在其所属刻到达时揭示，直观呈现双方真实攻击间隔；HUD 实时渲染 HP/攻防速/行动槽/buff；悬停技能或 buff 可查看详细说明。完整规则、公式与配置指南见 [`docs/GAME_SPEC.md`](docs/GAME_SPEC.md)；
9. **确定性契约**：名字与配置不变 ⇒ 派生数据与对战结果永远不变，与进程、机器、请求次数、输入顺序无关（有测试守护，见 `tests/test_determinism.py`）。
...
| 变量共鸣（概率/变量池/倍率/可共鸣类型） | `config/game/skills.json` 的 `variable_link` |
| 称号结构、字段池与字段加成 | `config/game/titles.json`（structures + prefixes/cores/suffixes，各字段 bonus） |
