...
另有**创意工坊**（`/workshop.html`，独立管理页）：可视化编辑全部配置 JSON，
草稿可「试运行」打一场（不落盘），「保存并生效」经校验后写盘并热重载，无需重启。

## 它是如何工作的

1. **归一化**：名字去除首尾空白并（默认）忽略大小写（规则见 `config/game/system.json`）；
2. **派生**：`md5(归一化名字)` 作为 splitmix64 PRNG 的种子，按固定顺序投掷六维属性
   （高斯分布；命/攻为 ×100 整数量纲 20000/1500，防御减半 750，速度为小整数 10）
   与 2–3 个技能、组合式称号；
...
6. **称号生成与加成**：按权重抽取结构与各字段（名称/描述/属性加成同条目，如「候补」攻击-100），
   拼接成如「孤高术士·候补」的称号；
7. **对战**：以 **tick** 推进——每 tick 双方行动槽累加有效速度，满阈值（100）即可行动；
...
8. **逐条回放与富文本战报**：每条战报停顿一段可配置的时间（`battle.json` 的
   ...
   （变化时高亮）与行动槽数值（如 `90/100`）；悬停技能或 buff 可查看详细说明；
...
固定种子蒙特卡洛，检查各技能持有者胜率是否落在 45%–55%（当前基线 45.9%–54.6%）。
