...
4. **变量共鸣**：攻击类技能有概率（`variable_link`）与一项自身属性（攻/防/速/生命/暴击/闪避）建立共鸣，触发时按 `属性 × 倍率` 附加伤害；共鸣的变量与倍率都由 MD5 决定，并以技能名后缀体现（「淬毒之刃·坚」= 防御共鸣、「重击·命」= 生命共鸣）；
5. **称号生成与加成**：按权重抽取结构（单核心 / 前缀+核心 / 核心+后缀 / 双核心 / 全结构）与各字段，拼接成如「孤高术士·候补」的称号；每个字段自带小额属性加成（可为负，如「候补」攻击-1），卡牌展示聚合结果；
6. **对战**：以 **tick** 推进--每 tick 双方行动槽累加速度值，满阈值（100）即可行动；速度决定出手频率。伤害计算含暴击/闪避/技能触发/共鸣附伤/毒/眩晕/反伤/追击，行动刻耗尽按剩余生命比例判定；
7. **实时 HUD 与悬停详情**：每条战报附带双方状态快照，前端战斗面板实时渲染 HP 条、攻防速、平滑推进的行动槽与 buff 徽章；鼠标悬停技能或 buff 徽章可查看详细说明（机制描述 / 当前数值 / 风味文本）；
8. **确定性契约**：名字与配置不变 ⇒ 派生数据与对战结果永远不变，与进程、机器、请求次数、输入顺序无关（有测试守护，见 `tests/test_determinism.py`）。
