...
另有**可视化编辑器**（`/editor.html`，v3.1.0，独立管理页）：**技能以节点画布
编辑**——触发 → 条件（判断，出边分 pass/fail **分支**，含通用「比较值与值」
节点）→ 原子/结构（**loop 循环**为自适应虚线容器，一键包裹/解除），13 个
最小原子 + 9 种条件全部由引擎 schema 自描述驱动；**战斗页可编辑状态定义**
（策略字段 + 被动修饰表（含护盾池）+ 状态自身的**效果图**画布）；其余配置
分页签结构化编辑；草稿可「试运行」打一场（不落盘），「保存并生效」经校验后
写盘并热重载，无需重启。
