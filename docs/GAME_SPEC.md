...
> 同步更新本文档**。当前版本：**v3.9.0**（与 `config/game/system.json` 的 `version` 一致）。
...
| 技能 | 触发 | 基础效果（引擎真实值直显，v3.9.0 全池强化后） | 熟练度区间 |
| --- | --- | --- | --- |
| 重击 heavy_strike | 攻击时 | 攻击时，有 28.00% 概率，改为蓄力：下次攻击必定命中、暴击率 +50.00%，伤害提升至 300.00%，消耗本次行动。 | ×0.55–1.65 |
| 斩杀 execution | 攻击时 | 攻击时，有 66.00% 概率，敌方生命比例 ≤ 50.00%，使本次攻击 ×300.00%。 | ×0.7–1.5 |
| 嗜血 bloodthirst | 攻击时 | 攻击时，有 50.00% 概率，进入嗜血：2 次行动内，所造成伤害的 52.00% 转化为自身生命。 | ×0.75–1.35 |
| 淬毒之刃 venom | 攻击时 | 攻击时，有 60.00% 概率，命中则使敌方中毒：其每刻损失 92 点生命，持续 12 刻。 | ×0.8–1.3 |
| 眩晕重锤 stun_blow | 攻击时 | 攻击时，有 25.00% 概率，本次攻击无视敌方 85.00% 的防御，命中则令其 4 刻内无法行动。 | ×0.6–1.55 |
| 雷罚 thunder | 攻击时 | 替代攻击时，有 95.00% 概率，连续至多 10 次（逐次按 96.00% 衰减），打出 ×28.00% 的打击。 | ×0.85–1.15 |
| 斩断 sever | 敌方将行动 | 敌方即将行动时，有 17.00% 概率，使行动槽变化 -1000，打出 ×60.00% 的打击。 | ×0.65–1.5 |
| 疾影突袭 shadow_step | 命中后 | 命中时，有 95.00% 概率，本次为暴击时，使行动槽变化 6000，否则使行动槽变化 3800。 | ×0.7–1.45 |
| 千锤百炼 iron_hide | 受击时 | 受到伤害时，有 95.00% 概率，叠加一层锻痕：伤害减免 +14.00%，持续 20 刻。 | ×0.8–1.25 |
| 荆棘反甲 thorns | 受击时 | 受到伤害时，有 46.00% 概率，使所受伤害降低 22.00%，把被减免的伤害以 ×170.00% 打出。 | ×0.7–1.4 |
| 坚守壁垒 bulwark | 受击时 | 受到伤害时，自身生命比例 ≥ 100.00%，使所受伤害降低 50.00%，有 6.00% 概率，完全免疫本次伤害。 | ×0.6–1.6 |
| 以牙还牙 retribution | 被命中后/命中后 | 被命中时，有 58.00% 概率，记录该次伤害（至多 12 条），下次命中时追加打出；命中时，把记仇记录总和以 ×42.00% 打出。 | ×0.7–1.45 |
| 不屈意志 iron_will | 战斗开始/致命伤害 | 不屈：濒死时有概率重生（每次触发后概率减半，永不耗尽）；受到致命伤害时，有 0.52 × (0.5)^不屈层数 概率，回复自身最大生命 60.00% 的生命，标记「不屈」：叠层，不屈意志已触发。 | ×0.5–1.5 |
| 回春术 spring_heal | 行动开始 | 行动开始时，有 30.00% 概率，回复自身 1300 点生命，获得回春印记：每 20 刻回复 340 点生命，持续 30 刻。 | ×0.75–1.4 |
| 净化 purify | 行动开始 | 行动开始时，有 26.00% 概率，驱散双方所有状态（每驱散 1 种回复 380 点生命），并回复 430 点生命。 | ×0.7–1.5 |
| 背水一战 last_stand | 攻击前 | （每场战斗一次），自身生命比例 < 60.00%，攻击 +20.00% 且速度 +20.00%，持续到战斗结束。 | ×0.8–1.3 |
| 乘胜追击 momentum | 命中后/攻击落空 | 命中时，有 95.00% 概率，获得 1 层乘胜（每层伤害 +13.00%，至多 10 层；攻击落空时清零）；攻击落空时，操控自身乘胜：叠层 -20。 | ×0.85–1.2 |
| 燃血轰击 overload | 攻击时 | 攻击时，有 70.00% 概率，消耗自身 12.00% 生命，使本次攻击 ×390.00%。 | ×0.65–1.55 |
| 破甲连打 rupture | 攻击时 | 攻击时，有 72.00% 概率，命中则使对方防御 −1050，持续 16 刻，至多叠加 6 层。 | ×0.75–1.35 |
| 撕裂 lacerate | 攻击时 | 攻击时，有 64.00% 概率，命中则撕裂敌方：其每次行动损失相当于攻击者攻击 40.00% 的生命，持续 12 刻。 | ×0.75–1.35 |
| 孤注一掷 all_in | 攻击时 | 攻击时，有 78.00% 概率，有 78.00% 概率，使本次攻击 ×210.00%，否则使本次攻击 ×40.00%。 | ×0.7–1.5 |
| 大器晚成 late_bloom | 行动后 | 每次行动完毕，有 32.00% 概率，速度 +220、攻击 +24（永久叠加）。 | ×0.85–1.15 |
| 洞悉 true_sight | 攻击时 | 攻击时，有 84.00% 概率，本次攻击无视敌方 100.00% 的防御，且暴击率 +12.00%。 | ×0.7–1.45 |
| 血契 blood_pact | 行动开始/行动后 | 行动开始时，有 80.00% 概率，消耗自身 12.00% 生命，本次攻击附带 85.00% 吸血，行动结束后按吸血量转化为攻击；每次行动完毕，把本次吸血量的 15.00% 永久转化为攻击。 | ×0.7–1.45 |
| 厚积薄发 grudge | 被命中后 | 被命中时，有 95.00% 概率，积攒 1 层怨念（持续 20 刻）：每层使伤害 +14.00%。 | ×0.75–1.4 |
| 洗礼 baptism | 攻击时 | 攻击时，有 42.00% 概率，回复自身 1200 点生命，回复敌方 1200 点生命，命中则刻下审判标记：每层独立倒数 12 刻，到期各造成 2200 点伤害（可叠加、可驱散）。 | ×0.55–1.65 |
| 命运天平 fate_scale | 被命中后 | 被命中时，敌方生命比例 > 自身生命比例，有 27.00% 概率，与对方交换当前生命值（各自不超过上限），回复自身 240 点生命，否则消耗自身 840 生命。 | ×0.6–1.5 |
