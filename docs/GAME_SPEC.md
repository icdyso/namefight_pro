...
> 同步更新本文档**。当前版本：**v1.2.1**（与 `config/game/system.json` 的 `version` 一致）。
...
- **角色名头顶血条（v1.2.1）**：前端在每个角色名段头顶渲染无数字简易血条--
  蓝=当前生命、红=本次掉血、绿=本次回血、灰=空；数据 = 本条快照与
...
- **战报富文本**：阵营名红/蓝加粗（a=红方、b=蓝方）；技能名按 id 散列配色
  ...
  **角色名为「【称号】名字」且头顶挂无数字简易血条**（v1.2.1：蓝=当前、
