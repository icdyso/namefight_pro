...
> 同步更新本文档**。当前版本：**v0.10.0**（与 `config/game/system.json` 的 `version` 一致）。
...
1. 属性（配置顺序，每维一次**高斯投掷** `[min, max]`；非百分比属性取整）
2. 技能数量 k（**高斯**，[2, 3]）
3. 技能抽取（不放回加权，保持顺序）
...
属性随机于 v0.9.1 恢复（正态投掷）。

---

## 3. 属性（`game/attributes.json`）-- ×100 整数量纲 + 真实值直显（v0.10.0）

| 属性 | 基准 base | 投掷区间 | 战力权重 | emoji |
| --- | --- | --- | --- | --- |
| hp 生命 | 20000 | 16000–24000 | 1 | ❤️ |
| atk 攻击 | 1500 | 1000–2000 | 4 | ⚔️ |
| def 防御 | 1500 | 1000–2000 | 3 | 🛡️ |
| spd 速度 | 1500 | 1000–2000 | 2 | 👟 |
| crit 暴击 | 12% | 5–20 | 2 | 🎯 |
| dodge 闪避 | 10% | 5–15 | 2 | 🌀 |

- **属性 = 高斯投掷值 + 称号加成**（下限 1）：投掷在 `[min, max]` 内正态分布
  （均值 = 区间中点 ≈ base，σ = 区间宽 ÷ 4，截断）；非百分比属性**取整**，
  crit/dodge 为百分数（浮点）；称号加成可把面板推过区间；
- **显示 = 引擎真实值**：v0.9.1 的白板 100 换算（display_ref）已删除，
  卡牌 / HUD / 快照 / 战报数值 / 共鸣公式一律直显；
- 属性条按投掷区间线性映射到 **8%–92%**（永不空条/满条）；
- 量纲相关数值示例：破甲每层 460（防 1500 量纲）、行动槽推进 3300/5600
  （阈值 10000 量纲）、大器晚成每动 +97（速 1500 量纲）、毒伤 380 /
  回春 550 等以命 20000 量纲的绝对值。
...
`on_defense` / `on_turn_start` / `passive`。共 **25** 个技能。
**各技能基础触发率按强度互不相同**（强效低频：斩断 10% … 弱效高频：
乘胜追击 80%），任何触发率永不超过 100%（个性化后截断 [2%, 95%]）。
v0.10.0 在防御倒数免伤 + 生命翻倍的新环境下四轮重调
（10000 场蒙特卡洛，持有者胜率 46.5%–54.6%）。

| 技能 | 触发 | 基础效果（引擎真实值） | 熟练度区间 |
| --- | --- | --- | --- |
| 重击 heavy_strike | on_attack | 16% 概率蓄力（占一次行动）：下次行动必定命中、暴击+40、伤害 ×3.0 | ×0.55–1.65 |
| 斩杀 execution | on_attack | 37% 概率，敌方 HP≤35% 时伤害 ×2.9 | ×0.7–1.5 |
| 嗜血 bloodthirst | on_attack | 25% 概率获得 2 次行动内 26% 吸血 | ×0.75–1.35 |
| 淬毒之刃 venom | on_attack | 30% 概率中毒：每动 380 点，持续 12 刻 | ×0.8–1.3 |
...
| 斩断 sever | on_defense | 敌方即将行动时 10% 概率打断其回合并反击 ×0.45，其行动槽倒退 900 | ×0.65–1.5 |
| 疾影突袭 shadow_step | on_attack | 命中后 62% 概率行动槽 +3300（暴击 +5600；阈值 10000 量纲） | ×0.7–1.45 |
...
| 不屈意志 iron_will | passive | 致命伤 44% 概率回复 20% 生命；每次触发后概率 ×0.3 指数衰减 | ×0.5–1.5 |
| 回春术 spring_heal | on_turn_start | 15% 概率回复 550 点 + 回春印记（每 10 刻回 150 点，持续 30 刻） | ×0.75–1.4 |
| 净化 purify | on_turn_start | 26% 概率驱散双方所有标记与增益减益（每种回 320 点）+ 回 260 点 | ×0.7–1.5 |
...
| 燃血轰击 overload | on_attack | 35% 概率消耗 5% 最大生命，本次伤害 ×2.4 | ×0.65–1.55 |
| 破甲连打 rupture | on_attack | 36% 概率击碎护甲：防御 −460（可叠 3 层），持续 16 刻 | ×0.75–1.35 |
| 撕裂 lacerate | on_attack | 32% 概率流血：每动损失攻击者攻击 20% 的生命，持续 12 刻 | ×0.75–1.35 |
| 孤注一掷 all_in | on_attack | 39% 概率伤害 ×2.1，否则 ×0.5 | ×0.7–1.5 |
| 大器晚成 late_bloom | passive | 每次行动后 28% 概率速度 +97，无上限叠加 | ×0.85–1.15 |
| 洞悉 true_sight | on_attack | 42% 概率该击无视全部防御且暴击 +5 | ×0.7–1.45 |
| 血契 blood_pact | on_turn_start | 40% 概率献祭 6% 最大生命：该次攻击 80% 吸血，行动后按吸血量的 10% 永久加攻击 | ×0.7–1.45 |
...
- **对战实时技能数据**：主句数值随双方当前快照逐刻变化（后端提供 `live_text`
  ...
  **按标记顺序**重算，与引擎 `resonance_coeff + apply_resonance` 完全一致，
  有测试保障）；
...
每条目同时含 `name`/`desc`/`bonus`）。

**称号加成**（×100 整数量纲，攻/防/速 ±100 或 ±200，HP +400~800）：

- 前缀：单项（攻+100 / 防+100 / 速+100 / 命+400 / 暴+1 / 闪+1）
- 核心：单项（攻+100 / 命+600 / 暴+1 / 防+100 / 速+100；战神 攻+200、神医 命+800）
- 后缀：无双 攻+100暴+1 / 觉醒 暴+1闪+1 / 再临 命+600 / 传说 攻+200 /
  亲卫 防+200 / 见习 暴+1 / **候补 攻−100** / **退役 速−100 防+100**
  （负加成为刻意设计）
...
flowchart TD
    ...
    G --> R[行动槽 >= 10000 者可行动<br/>按槽余量降序、内部序依次]
    R --> A0{斩断判定<br/>（敌方技能打断）}
...
- 不屈意志：每次致命伤独立掷骰（当前概率），成功则生命回到 `value × 最大生命`
  （取整），概率 ×decay 指数衰减，可多次触发直至概率衰减殆尽；
...
| 常数 | 值 |
| --- | --- |
| gauge_threshold | 10000（行动槽阈值，每刻 += 有效速度） |
| atk_factor | 1.0 |
...
行动间隔 ≈ `10000 ÷ 有效速度` 刻（速度 1500 ≈ 每 7 刻一动）。
...
- `md5_variance{value[lo,hi]}`：数值高斯扰动区间；
...
战斗常数见第 6 节表；`battle_log`（全部战报模板，覆盖引擎全部模板 id，有测试）、
`buffs`（16 种 buff 的 name/detail/desc）、`playback.message_delay_ms`
（前端每条战报停顿时长，>=16）。
示例：更快战斗 -> `"gauge_threshold": 8000`；更慢战报 -> `"playback": {"message_delay_ms": 500}`。
...
- 属性正态投掷（高斯，均值 = 区间中点，σ = 宽 ÷ 4，截断）：HP [16000,24000] /
  ATK [1000,2000] / DEF [1000,2000] / SPD [1000,2000] / CRIT [5,20]% /
  DODGE [5,15]%；非百分比属性取整；**显示一律为引擎真实值**
- 技能 25 个、每名斗士 2–3 个（高斯）；**熟练度** 0–100 高斯抽样，按技能区间
  缩放触发率（或条件型效果值），熟练度行显示最终触发率（≤ 100%）；
  **变数**每技能 2 槽位 × 25% 出现率，模式 own 10 : enemy 3 : difference 2 :
  sum 1，归一化倍率 0.25–0.65；词缀 前缀 50% / 后缀 40%
- 数值类随机全部高斯；**计算结果取整**（多步浮点只在最终应用时取整一次）
- 行动槽阈值 10000 / 最大 600 刻 / 暴击 ×1.8 / 高斯浮动 ±15%（截断）/
  伤害下限 100 / 免伤率 = DEF/(DEF+2500)
- 回放 320ms/条（`playback.message_delay_ms` 可配置）；极速接口 /api/battle/fast
- 数值精度：后台全浮点；展示层百分数 2 位小数、其余数值整数
- 暴击上限 100%、闪避上限 60%；持续状态以刻为期（毒/血/晕/破甲/回春/怨念/锻痕），
  毒/流血伤害在拥有者行动时结算；不屈意志可多次触发、概率指数衰减
- 超时：按剩余生命比例判定，完全相同则平局
- 平衡基线：10000 场蒙特卡洛下技能持有者胜率 46.5%–54.6%
