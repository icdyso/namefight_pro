...
> 同步更新本文档**。当前版本：**v3.8.1**（与 `config/game/system.json` 的 `version` 一致）。
...
| 命运天平 fate_scale | on_hit_taken | 被命中后：敌方生命比例更高时 11.5% 概率**交换双方当前生命**（各自不超过上限）并回复 120 点，否则**必然**自扣 420 点（保底 1 点） | 触发率 ×0.6–1.5 |
