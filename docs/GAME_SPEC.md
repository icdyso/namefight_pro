...
> 同步更新本文档**。当前版本：**v1.1.0**（与 `config/game/system.json` 的 `version` 一致）。
...
- 量纲（v1.0.0）：命/攻保持 ×100 整数量纲；**防御减半**；**速度与行动条
  ...
  行动槽推进 28/48（阈值 100 量纲）、斩断倒退 9、大器晚成每动 +1 速/+10 攻
  （速 10/攻 1500 量纲）、毒伤 38/刻 / 回春 550 等以命 20000 量纲的绝对值。
...
| 疾影突袭 shadow_step | on_attack | 命中后 62% 概率行动槽 +28（暴击 +48；阈值 100 量纲） | ×0.7–1.45 |
...
| 大器晚成 late_bloom | passive | 每次行动后 16% 概率速度 +1、攻击 +10（v1.1.0 加攻），无上限叠加 | ×0.85–1.15 |
...
flowchart TD
    ...
    T -->|是| TK[每刻开始: 毒发结算<br/>回春回复/到期层清理（怨念/锻痕）] --> G[存活方行动槽 += 有效速度]
...
- 属性三角形分布投掷（两均匀数取均值，密度中点最高、端点无截断堆积）：
  HP [10000,30000] / ATK [1000,2000] / DEF [500,1000] / SPD [5,15] /
...
- 行动槽阈值 100 / 最大 600 刻 / 暴击 ×1.8 / 三角形浮动 [0.85,1.15]（天然
