...
> 同步更新本文档**。当前版本：**v3.11.0**（与 `config/game/system.json` 的 `version` 一致）。
...
| 斩杀 execution | 攻击时 | 攻击时，有 66.00% 概率，敌方生命比例 ≤ 50.00%，使本次攻击 ×300.00%。 | ×0.25–2.5 |
| 嗜血 bloodthirst | 攻击时 | 攻击时，有 50.00% 概率，进入嗜血：2 次行动内，所造成伤害的 52.00% 转化为自身生命。 | ×0.25–2.5 |
| 淬毒之刃 venom | 攻击时 | 攻击时，有 60.00% 概率，命中则使敌方中毒：其每刻损失 92 点生命，持续 12 刻。 | ×0.25–2.5 |
| 眩晕重锤 stun_blow | 攻击时 | 攻击时，有 25.00% 概率，本次攻击无视敌方 85.00% 的防御，命中则令其 4 刻内无法行动。 | ×0.25–2.5 |
| 雷罚 thunder | 攻击时 | 替代攻击时，有 95.00% 概率，连续至多 10 次（逐次按 96.00% 衰减），打出 ×28.00% 的打击。 | ×0.25–2.5 |
| 斩断 sever | 敌方将行动 | 敌方即将行动时，有 17.00% 概率，使行动槽变化 -1000，打出 ×60.00% 的打击。 | ×0.25–2.5 |
| 疾影突袭 shadow_step | 命中后 | 命中时，有 95.00% 概率，本次为暴击时，使行动槽变化 6000，否则使行动槽变化 3800。 | ×0.25–2.5 |
| 千锤百炼 iron_hide | 受击时 | 受到伤害时，有 95.00% 概率，叠加一层锻痕：伤害减免 +14.00%，持续 20 刻。 | ×0.25–2.5 |
...
| 以牙还牙 retribution | 被命中后/命中后 | 被命中时，有 58.00% 概率，记录该次伤害（至多 12 条），下次命中时追加打出；命中时，把记仇记录总和以 ×42.00% 打出。 | ×0.25–2.5 |
...
| 乘胜追击 momentum | 命中后/攻击落空 | 命中时，有 95.00% 概率，获得 1 层乘胜（每层伤害 +13.00%，至多 10 层；攻击落空时清零）；攻击落空时，操控自身乘胜：叠层 -20。 | ×0.25–2.5 |
| 燃血轰击 overload | 攻击时 | 攻击时，有 70.00% 概率，消耗自身 12.00% 生命，使本次攻击 ×390.00%。 | ×0.25–2.5 |
| 破甲连打 rupture | 攻击时 | 攻击时，有 72.00% 概率，命中则使对方防御 −1050，持续 16 刻，至多叠加 6 层。 | ×0.25–2.5 |
| 撕裂 lacerate | 攻击时 | 攻击时，有 64.00% 概率，命中则撕裂敌方：其每次行动损失相当于攻击者攻击 40.00% 的生命，持续 12 刻。 | ×0.25–2.5 |
| 孤注一掷 all_in | 攻击时 | 攻击时，有 78.00% 概率，有 78.00% 概率，使本次攻击 ×210.00%，否则使本次攻击 ×40.00%。 | ×0.25–2.5 |
| 大器晚成 late_bloom | 行动后 | 每次行动完毕，有 32.00% 概率，速度 +220、攻击 +24（永久叠加）。 | ×0.25–2.5 |
| 洞悉 true_sight | 攻击时 | 攻击时，有 84.00% 概率，本次攻击无视敌方 100.00% 的防御，且暴击率 +12.00%。 | ×0.25–2.5 |
| 血契 blood_pact | 行动开始/行动后 | 行动开始时，有 80.00% 概率，消耗自身 12.00% 生命，本次攻击附带 85.00% 吸血，行动结束后按吸血量转化为攻击；每次行动完毕，把本次吸血量的 15.00% 永久转化为攻击。 | ×0.25–2.5 |
| 厚积薄发 grudge | 被命中后 | 被命中时，有 95.00% 概率，积攒 1 层怨念（持续 20 刻）：每层使伤害 +14.00%。 | ×0.25–2.5 |
