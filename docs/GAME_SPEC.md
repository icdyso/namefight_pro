...
> 同步更新本文档**。当前版本：**v3.4.0**（与 `config/game/system.json` 的 `version` 一致）。
...
| 洗礼 baptism | on_attack | 22% 概率自身与敌方各回复 600 点，命中则给敌方刻 1 层审判标记（每层 900 点，独立倒数 12 刻，到期各罚一次、可致死；可驱散） | 效果 ×0.55–1.65 |
...
- **审判标记（v3.4.0）**：layers 模式的「时间炸弹」型状态——每层独立倒数，
  ...
  （洗礼：每层 900 点体力流失，can_kill 可致死，判定先于 on_status_lose）；
  被驱散的层不触发（走 on_status_lose 路径）；
...
- **可视化编辑器**（v2.0.0 `/editor.html`；v3.0.0 升级为双画布）：
  ...
  **loop 为自适应容器**——虚线框按子树真实尺寸包裹循环体，拖动容器整体
  ...
  状态定义表单（策略字段 / params 规格 / mods 表）；其余页签为
