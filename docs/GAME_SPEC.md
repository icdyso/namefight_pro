...
> 同步更新本文档**。当前版本：**v0.9.1**（与 `config/game/system.json` 的 `version` 一致）。
...
- **属性正态投掷 + 白板 100 显示**：六维属性在各自 `[min, max]` 区间内**高斯投掷**
  （引擎使用原始量纲：命 100 / 攻 13 / 防 5 / 速 10）；显示层经 `display_ref`
  把面板换算为「**满投掷 = 100**」的白板单位（称号/加成可超过 100）--
  **只影响展示，数值计算不变**；
- **熟练度系统**：每个技能实例附带 0–100 的熟练度，按技能各自的区间缩放触发概率
  （条件触发型技能缩放效果值），取代 v0.8 的概率高斯扰动；
  熟练度行直接显示**最终触发率**（各技能基础触发率按强度互不相同，且永不超 100%）；
...
flowchart LR
    ...
    C --> D[斗士派生<br/>属性高斯投掷/技能/称号]
    D --> E[技能个性化<br/>熟练度/高斯扰动/词缀/双变数]
    E --> F{开始对战?}
    F -->|是| G[对战种子 = md5 排序后双方名字]
    G --> H[tick 战斗循环]
    H --> I[结构化战报 + 状态快照<br/>（显示层白板100换算）]
...
### 2.4 派生主 PRNG 消耗顺序（v0.9.1，固定，改变即 breaking）

种子：`md5(归一化名字)` -> splitmix64。

1. 属性（配置顺序，每维一次**高斯投掷** `[min, max]`）
2. 技能数量 k（**高斯**，[2, 3]）
3. 技能抽取（不放回加权，保持顺序）
...
**元素系统已删除**（v0.9.1，纯装饰性身份标识整体移除）；
**稀有度系统已删除**（v0.5.0）；属性随机于 v0.9.1 恢复（正态投掷）。

---

## 3. 属性（`game/attributes.json`）-- 原始量纲 + 白板 100 显示（v0.9.1）

| 属性 | 基准 base | 投掷区间 | 显示参照 display_ref | 战力权重 | emoji |
| --- | --- | --- | --- | --- | --- |
| hp 生命 | 100 | 80–120 | 120 | 1 | ❤️ |
| atk 攻击 | 13 | 10–16 | 16 | 4 | ⚔️ |
| def 防御 | 5 | 2–8 | 8 | 3 | 🛡️ |
| spd 速度 | 10 | 6–14 | 14 | 2 | 👟 |
| crit 暴击 | 12% | 5–20 | -（百分数直显） | 2 | 🎯 |
| dodge 闪避 | 10% | 5–15 | -（百分数直显） | 2 | 🌀 |

- **属性 = 高斯投掷值 + 称号加成**（下限 1）：投掷在 `[min, max]` 内正态分布
  （均值 = 区间中点 ≈ base，σ = 区间宽 ÷ 4，截断）；称号加成可把面板推过区间；
- **显示层白板 100**：`显示值 = 引擎值 ÷ display_ref × 100`（display_ref =
  满投掷 = max）--只影响展示（卡牌 / HUD / 快照 / 战报数值 / 共鸣公式），
  **引擎与配置一律使用原始量纲**；白板满投掷显示为 100，称号/加成下可超过 100；
  crit/dodge 本身即百分数，不参与换算；
- 白板（中位）战力 ≈ 231（hp100×1 + atk13×4 + def5×3 + spd10×2 + 暴12×2 + 闪10×2）；
- 量纲相关数值示例：破甲每层 1.7（防 5 量纲）、行动槽推进 33/56（阈值 100 量纲）、
  大器晚成每动 +0.34（速 10 量纲）、血契转化 7.8%（吸血量 -> 攻 13 量纲）；
  毒伤 4.5 / 回春 4.5 / 净化 2.2 等以命 100 量纲的绝对值。
...
（v0.8 的 27 个中删除 疾风步/心眼，重做 20 个，微调其余）；v0.9.1 数值回退到
原始量纲并按恢复属性随机后的环境重新调参（10000 场蒙特卡洛，
持有者胜率 45.6%–54.8%）。**各技能基础触发率按强度互不相同**
（强效低频：斩断 10% … 弱效高频：大器晚成 84%），任何触发率永不超过 100%
（个性化后截断 [2%, 95%]）。

| 技能 | 触发 | 基础效果（引擎值，原始量纲） | 熟练度区间 |
| --- | --- | --- | --- |
| 重击 heavy_strike | on_attack | 16% 概率蓄力（占一次行动）：下次行动必定命中、暴击+30、伤害 ×2.3 | ×0.55–1.65 |
| 斩杀 execution | on_attack | 37% 概率，敌方 HP≤35% 时伤害 ×2.9 | ×0.7–1.5 |
| 嗜血 bloodthirst | on_attack | 25% 概率获得 2 次行动内 26% 吸血 | ×0.75–1.35 |
| 淬毒之刃 venom | on_attack | 30% 概率中毒：每动 4.5 点，持续 12 刻 | ×0.8–1.3 |
| 眩晕重锤 stun_blow | on_attack | 17% 概率破防重击：该击无视 35% 防御，命中眩晕 6 刻 | ×0.6–1.55 |
| 雷罚 thunder | on_attack | 78% 概率替换攻击：连续真实伤害（每道攻击 24%，后续每道概率 ×0.93 衰减，至多 9 道） | ×0.85–1.15 |
| 斩断 sever | on_defense | 敌方即将行动时 10% 概率打断其回合并反击 ×0.5，其行动槽倒退 12 | ×0.65–1.5 |
| 疾影突袭 shadow_step | on_attack | 命中后 62% 概率行动槽 +33（暴击 +56；阈值 100 量纲） | ×0.7–1.45 |
| 千锤百炼 iron_hide | on_defense | 受击时 65% 概率叠锻痕：每层减伤 10%，持续 30 刻，无限叠 | ×0.8–1.25 |
| 荆棘反甲 thorns | on_defense | 受伤时 31% 概率免伤 18% 并以 ×1.25 反弹该部分 | ×0.7–1.4 |
| 坚守壁垒 bulwark | on_defense | HP≥60% 时受伤 −28%，且 9% 概率完全免疫 | 免疫 ×0.6–1.6 |
| 以牙还牙 retribution | on_defense | 受击时 29% 概率记录伤害（至多 6 条）：下次命中以 ×0.6 追加打出总和 | ×0.7–1.45 |
| 不屈意志 iron_will | passive | 致命伤 52% 概率回复 25% 生命；每次触发后概率 ×0.3 指数衰减 | ×0.5–1.5 |
| 回春术 spring_heal | on_turn_start | 15% 概率回复 4.5 点 + 回春印记（每 10 刻回 1.2 点，持续 30 刻） | ×0.75–1.4 |
| 净化 purify | on_turn_start | 38% 概率驱散双方所有标记与增益减益（每种回 3.2 点）+ 回 2.2 点 | ×0.7–1.5 |
| 背水一战 last_stand | passive | HP<30% 时攻击 +24%、速度 +22%（一次触发，持续到结束） | 效果 ×0.8–1.3 |
...
| 破甲连打 rupture | on_attack | 36% 概率击碎护甲：防御 −1.7（可叠 3 层），持续 16 刻 | ×0.75–1.35 |
| 撕裂 lacerate | on_attack | 32% 概率流血：每动损失攻击者攻击 26% 的生命，持续 12 刻 | ×0.75–1.35 |
| 孤注一掷 all_in | on_attack | 39% 概率伤害 ×2.1，否则 ×0.5 | ×0.7–1.5 |
| 大器晚成 late_bloom | passive | 每次行动后 84% 概率速度 +0.34，无上限叠加 | ×0.85–1.15 |
| 洞悉 true_sight | on_attack | 42% 概率该击无视全部防御且暴击 +12 | ×0.7–1.45 |
| 血契 blood_pact | on_turn_start | 40% 概率献祭 8% 最大生命：该次攻击 80% 吸血，行动后按吸血量的 7.8% 永久加攻击 | ×0.7–1.45 |
| 怨念深重 grudge | on_defense | 被命中时 60% 概率 +1 层怨念（持续 20 刻）：每层伤害 +7%，无上限 | ×0.75–1.4 |

调参记录（v0.9.1，恢复属性随机后的环境）：防御在 [2,8] 浮动放大了防穿与续航
价值（洞悉/回春/嗜血/斩断首测 56%–59%），四轮下调后全部回到 45.6%–54.8%；
怨念首测 43.5% 上调；触发率顺带按强度去重（互不相同）。
v0.9.0 初版记录：撕裂首测 71.3%（流血误用面板攻击）；重击 64.3% 三轮下调；
斩断 65.3% → 54%；疾影突袭 35.3% → 47%。
...
- 变量池与倍率（**高斯**）不变：atk 3 / def 2 / spd 3 / hp 2 / crit 1 / dodge 1，
  ...
  变量自身，v0.9.1 起攻↔攻、速↔速等，【己方-对方】⚔️ 即双方攻击之差）。
...
**称号加成**（v0.9.1 回退到原始量纲：攻/防/速 ±1 或 ±2，HP 与暴击/闪避不变）：

- 前缀：单项（攻+1 / 防+1 / 速+1 / 命+2 / 暴+1 / 闪+1）
- 核心：单项（攻+1 / 命+3 / 暴+1 / 防+1 / 速+1；战神 攻+2、医师 命+4）
- 后缀：无双 攻+1暴+1 / 觉醒 暴+1闪+1 / 再临 命+3 / 传说 攻+2 / 亲卫 防+2 /
  见习 暴+1 / **候补 攻−1** / **退役 速−1 防+1**（负加成为刻意设计）

卡牌聚合加成行按 `display_factor` 显示（如 攻+1 显示为 攻+6）。
...
**伤害公式**（全程浮点，不做中间取整；引擎使用原始量纲，atk_factor /
defense_factor 均为 1.0，保留系数以便日后调整量纲）：

```
有效防御 = max(0, 敌方DEF − 破甲每层值 × 层数)          // 破甲到期后清零
raw   = 有效ATK × atk_factor(1.0) × 高斯浮动(截断[0.85,1.15]) × 暴击倍率(1.8) × 技能倍率
dmg   = max( 1, raw − 有效防御 × (1 − 穿透率) × defense_factor(1.0) )
...
| 常数 | 值 |
| --- | --- |
| gauge_threshold | 100（行动槽阈值，每刻 += 有效速度） |
| atk_factor / defense_factor | 1.0 / 1.0（原始量纲直通） |
| max_ticks | 600 |
| crit_multiplier | 1.8 |
...
行动间隔 ≈ `100 ÷ 有效速度` 刻（速度 10 ≈ 每 10 刻一动）。
...
`{"ref": 注册名, "id": 条目id}` 传递（注册名：skill/attr/stat_word），
渲染时查 locale。**战报与快照中的全部数值均为白板 100 显示单位**
（伤害/回复/生命按命量纲 ÷1.2，破甲按防量纲，叠速按速量纲换算；
引擎内部仍用原始量纲，换算只发生在展示边界）。`state` 按输入位置 a/b，含
...
## 9. `config/game/*.json` 调整指南（共 5 个文件）
...
### attributes.json —— 属性（正态投掷 + 白板 100 显示）
`attributes[] {id, base, min, max, format, power_weight, display_ref}`：
属性在 `[min, max]` 内**高斯投掷**（base 为名义基准，供共鸣归一化等使用）；
`display_ref` 为显示换算参照（= 满投掷，显示值 = 值 ÷ display_ref × 100，
0 表示原值直显）——**只影响展示**；引擎必需 id：hp/atk/def/spd/crit/dodge。
...
与 `prefixes/cores/suffixes[] {id,weight,bonus{属性:增量}}`（bonus 为原始量纲）。
...
## 10. `config/locales/<lang>/*.json` 调整指南（共 8 个文件）
...
新增语言：复制 `locales/zh/` 八个文件翻译，加入 `available_locales`，重启生效。
...
- **展示精度**：百分数 2 位小数（`fmtPct`），其余数值整数（`fmtInt`）——
  与后端 `format_pct` / `format_num` 同一规则；属性值显示白板 100 单位
  （满投掷 = 100，称号/加成可超过 100）；
- **对战实时技能数据**：共鸣技能卡的每个变数位（至多 2 个）按双方快照逐刻重算
  （`link_calc` 列表 + `\u0001` 标记按序替换），数值变化时卡牌闪烁；
- **简易显示模式**：页眉开关（记忆 localStorage），隐藏技能描述中的共鸣公式；
  切换后实时数值保持（按最近快照恢复）；
- **技能卡**：名称行右侧显示熟练度（如「熟练度 63：触发率 36.52%」，永不超过
  100%）；自然语言描述为主行，风味短句为副行；悬停显示完整说明与词缀明细；
  元素徽章已随元素系统移除（v0.9.1）；
...
- 属性正态投掷（高斯，均值 = 区间中点，σ = 宽 ÷ 4，截断）：HP [80,120] /
  ATK [10,16] / DEF [2,8] / SPD [6,14] / CRIT [5,20]% / DODGE [5,15]%；
  **引擎使用原始量纲**，显示层按 display_ref（= max，满投掷 100）换算，
  crit/dodge 百分数直显；元素系统已删除
- 技能 25 个、每名斗士 2–3 个（高斯）；**熟练度** 0–100 高斯抽样，按技能区间
  缩放触发率（或条件型效果值），熟练度行显示最终触发率（≤ 100%），
  基础触发率按强度互不相同（10%–84%）；
  **变数**每技能 2 槽位 × 25% 出现率（双变数率 ≈ 6%），模式 own 10 : enemy 3 :
  difference 2 : sum 1，归一化倍率 0.25–0.65（差值/并值与同名属性运算）；
  词缀 前缀 50% / 后缀 40%（修正值高斯缩放 [0.7, 1.3]）
- 数值类随机全部高斯（σ = 区间宽 ÷ 4，截断区间）
- 行动槽阈值 100 / 最大 600 刻 / 暴击 ×1.8 / 高斯浮动 ±15%（截断）/ 伤害下限 1
  / atk_factor = defense_factor = 1.0（原始量纲直通）
- 回放 255ms/刻（原速 1/3）；极速接口 /api/battle/fast
- 数值精度：后台全浮点；展示层百分数 2 位小数、其余数值整数；
  战报/快照数值均为白板 100 显示单位
- 暴击上限 100%、闪避上限 60%；持续状态以刻为期（毒/血/晕/破甲/回春/怨念/锻痕），
  毒/血流血伤害在拥有者行动时结算；不屈意志可多次触发、概率指数衰减
- 超时：按剩余生命比例判定，完全相同则平局
- 平衡基线：10000 场蒙特卡洛下技能持有者胜率 45.6%–54.8%
