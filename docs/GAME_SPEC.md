...
> 同步更新本文档**。当前版本：**v1.0.0**（与 `config/game/system.json` 的 `version` 一致）。
...
- **数值抽样**（属性投掷、技能数量、熟练度、个性化扰动倍率、共鸣倍率、伤害浮动）：
  **高斯分布** `DetRng.next_gaussian(lo, hi)`--Box-Muller 变换（每次消耗两个
  ...
  离散数值（技能数量、熟练度 0–100，v1.0.0 起）用 `next_gaussian_range`
  （±0.5 扩展后取整）。
...
## 3. 属性（`game/attributes.json`）-- 真实值直显（v0.10.0）+ 量纲调整（v1.0.0）
...
- 量纲（v1.0.0）：命/攻保持 ×100 整数量纲；**防御减半**；**速度与行动条
  回退旧版小量纲**（阈值 100）。量纲相关数值示例：破甲每层 340（防 750 量纲）、
  行动槽推进 33/56（阈值 100 量纲）、斩断倒退 9、大器晚成每动 +1（速 10 量纲）、
  毒伤 380 / 回春 550 等以命 20000 量纲的绝对值。
...
| 斩断 sever | on_defense | 敌方即将行动时 8.5% 概率打断其回合并反击 ×0.45，其行动槽倒退 9 | ×0.65–1.5 |
...
| 坚守壁垒 bulwark | on_defense | HP≥60% 时受伤 −25%，且 9% 概率完全免疫 | 免疫 ×0.6–1.6 |
...
| 背水一战 last_stand | passive | HP<30% 时攻击 +21%、速度 +20%（一次触发，持续到结束） | 效果 ×0.8–1.3 |
...
| 大器晚成 late_bloom | passive | 每次行动后 27% 概率速度 +1，无上限叠加 | ×0.85–1.15 |
| 洞悉 true_sight | on_attack | 42% 概率该击无视全部防御且暴击 +5 | ×0.7–1.45 |
| 血契 blood_pact | on_turn_start | 40% 概率献祭 6% 最大生命：该次攻击 68% 吸血，行动后按吸血量的 10% 永久加攻击 | ×0.7–1.45 |
...
- **熟练度 `mastery`**：**高斯投掷** 0–100（v1.0.0 起 `next_gaussian_range`，
  集中于 50；随机消耗方式不变），按技能各自的
...
**称号加成**（与属性同量纲：攻 ±100/±200、防 ±50/±100、速 ±1、命 +400~800）：

- 前缀：单项（攻+100 / 防+50 / 速+1 / 命+400 / 暴+1 / 闪+1）
- 核心：单项（攻+100 / 命+600 / 暴+1 / 防+50 / 速+1；战神 攻+200、神医 命+800）
- 后缀：无双 攻+100暴+1 / 觉醒 暴+1闪+1 / 再临 命+600 / 传说 攻+200 /
  亲卫 防+100 / 见习 暴+1 / **候补 攻−100** / **退役 速−1 防+50**
  （负加成为刻意设计）
...
| 常数 | 值 |
| --- | --- |
| gauge_threshold | 100（行动槽阈值，每刻 += 有效速度；v1.0.0 回退旧版量纲） |
| atk_factor | 1.0 |
...
量纲：命/攻 ×100 整数、防御减半、速度小整数（v1.0.0）。
...
- 属性正态投掷（高斯，均值 = 区间中点，σ = 宽 ÷ 4，截断）：HP [16000,24000] /
  ATK [1000,2000] / DEF [500,1000] / SPD [6,14] / CRIT [5,20]% /
  DODGE [5,15]%；非百分比属性取整；**显示一律为引擎真实值**
- 技能 25 个、每名斗士 2–3 个（高斯）；**熟练度** 0–100 高斯投掷
  （`next_gaussian_range`），按技能区间缩放触发率（或条件型效果值），
  熟练度行显示最终触发率（≤ 100%）；
  **变数**每技能 2 槽位 × 25% 出现率，模式 own 10 : enemy 3 : difference 2 :
  sum 1，归一化倍率 0.25–0.65；词缀 前缀 50% / 后缀 40%
- 数值类随机全部高斯；**计算结果取整**（多步浮点只在最终应用时取整一次）
- 行动槽阈值 100 / 最大 600 刻 / 暴击 ×1.8 / 高斯浮动 ±15%（截断）/
...
- 平衡基线：6000 场蒙特卡洛下技能持有者胜率 45.9%–54.6%
